        ...
        if (posts.length) {
            ...
            <div class="list-group-item active"><strong>Search Results</strong> (${posts.length > 1 ? `${posts.length} items found` : '1 item found'})</div>
                ${posts.map(post => {
                    ...
                    <span class="text-muted small">by ${post.author.user} on ${postDate.getDate()}/${postDate.getMonth()}/${postDate.getFullYear()}</span>
                  </a>`
                }).join('')}
                </div>`)
        } else {
            this.resultsArea.innerHTML = `<p class="alert alert-danger text-cneter shadow-sm">Sorry, We could not find any results for that search</p>`
        }
        this.hideLoaderIcon()
